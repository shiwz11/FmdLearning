...
![Cumulative Returns of USD.AUD versus USD.CAD Strategy](https://cdn.jsdelivr.net/gh/shiwz11/pics_public@master/FMDPics/ATWSR-5-1.png)
...
**Example 5.4 : Mean Reversion Trading of Clendar Spreads**

As discussed above, the log market value of calendar spread portfolio with a long fra contract and a short near contract is simply $\gamma (T_1 - T_2)$, with $T_2 > T_1$. Hopefully, we can use mean-reverting $\gamma$ to generate trading signals.

```{matlab}
isGoddData = find(isfinite(gamma));
gammalag = lag(gamma(isGoddData), 1);
...
zScore = (gamma - ma) ./ matd;
```

Next, we need to pick a pair of contracts, far and near, on each historical day, based on three criteria:

1.The holding period for a pair of contracts is 3months (two time of the half-life);

2.Roll forward to the next pair of contracts 10 days before the current near contract's expiration;

3.The expiration dates of the near and far contracts are 1 year apart.

```{matlab}
isExpirData  = false(size(cl));
positions = zeros(size(cl));
isExpirData = isfinite(cl) & ~isfinite(fwdshift(1, cl));
holddays = 3*21;
numDaysStart = holddays + 10;
numDaysEnd = 10;
spreadMonth = 12; % No. months between far and near contracts.
for c=1:length(contracts) - spreadMOnth
  expireIdx = find(isExpireData(:, c));
  ...
  else % ensure next front month contract doesn't start unitl current one ends
    myStartIdx = endIdx + 1;
...
Finally, apply the linear mean reversion strategy to determine the true positons and calculate the unlevered daily reutrns of the portfolio.
...
## Future Intermarket Spreads

The most obvious candidate for pair trading futures is intermarket spreads between markets that are closely related.
...
- in computing the returns of a portfolio with two currency cross-rates, did you pay attention to whether they have the same quote currency, the same base currency, or neither? THe formulae for computing reutrns are not the same for all cases.

- Futures returns consist of two components:spot returns and roll reutrns.

- Cause of roll returns, mean reversion of the spot price may not induce mean reversion of the futures price.
...
4. Market manipulation by high-frequency trades.

## Test for Time Series Momentum

Time series momentum of a price series means that past reutrns are positively correlated with furure returns.We should find the optimal pair of past and future periods that gices the highest positive correlation and use that as our look-back and holding period for our momentum strategy.

**Finding Correlations between Returns of Different Time Frames** to determine the best time frame and the holding days.
...
## Tiem Series Strategies

From the result of the above program, we pick the best look-back and holding period of 250-20 days to perform a simple momentum strategy: buy (sell) the future if it has a positive (negative) 12-month return, and hold the position for one month.We make a trading decision everyday, and invest only one twenty-fifth of the total capital once.

**Example 6.1 TU Momentum Strategy**

Original codes contained in `TU_mom.m`

```{matlab}
lookback = 250;
holddays = 25;

longs = cl > bachshift(lookback, cl);
shorts = cl < backshift(lookback, cl);
...
for h=0:holddays-1
  long_lag = backshift(h, longs);
  log_lag(isnan(log_lag)) = false;
  log_lag = logical(long_lag);
...
There is also a classic momentum strategy called the Alexander FIlter which tell us to buy the daily return moves up at least $x$ percent, and then sell and go short if the price movce down at least $x$ percent from a subsequent high (Fama and Blume, 1966).


## Extracting Roll Returns through Future versus ETF Arbitrage

If futures' total returns = spot returns + roll returns, then an obvious way to extract roll return is buy the underlying asset and short the futures, if the roll return is negative (i.e., under contango); and vice versa if the roll return is positive (i.e., under backwardation).

### Volatility Futures versus Equity Index Futures: Redux

VX's roll returns can be as low as -50 percent annualized, at the same time, it is highly anticorrelated with ES, with a correlation coefficient of daily returns reaching -75 percent. Here we will make use of the large roll return mangnitude of VX, the small roll return magnitude of ES, and the anticorrelation of VX and ES to develop a momentum strategy. (Simon and Campasano, 2012):
...
2. If the price of the front contract of VX is lower than that of VIX by 0.1 point (backwardation) times the number of trading days untill settlement, buy 0.3906 froont contracts of VX and buy 1 front contract of ES. Hold for one day.

## Cross-Sectional Strategies

The third way to extract the roll returns in gutures: just buy a portfolio of futures in backwardation, and simultaneously short a portfolio of futures in contango.The hope is htat the reutrns of the spot prices cancel each other out (cause we belive commodities' spot prices are positively correlated with economic growth or some other macroeconomic indices), and left with the favorable roll returns.

Applying this strategy to US stocks, we cna just buy and hold stocks within the top decile of 12-month lagged returns for a month, and vice versa for the bottom decile.

**Example 6.2: Cross-Sectional Momentum Strategy for Stocks:**

The functions `smartsum` and `backshift` are available form `http://epchan.com/book2`. The code can be downloaded as `kentdaniel.m`
...
### Mutal Funds Asset Fire Sale and Forced Purchases

## Pros and Cons of Momentum Strategies


**Cons**:It is always harder to create profitable momentum strategies, and those that are profitable tend to have lower Sharpe ratios than mean-reversal strategies.

reason 1:As we can see so far, many established momentum strategies have long look-back and holding periods. So clearly the number of independent trading signals is few and far in between, which leads to a lower Sharpe ratio.

reason 2: Research by Daniel and Moskowitz on "momentum crashes" indicates that momentum strategies for futures or stocks tend to perform miserable for several years after a financial crissi (Daniel and Moskowitz, 2011).

reason 3: The duration over which momentum remains in force gets progressively shorter as more traders catch on to it.

**Pros**: 1.ease of risk management;

For mean-reverting strategies, their upside is limited by their natural profit cap, but their downside can be unlimited.For mometum strategies, their upside is unlimited while their downside is limited.

The thicker the tails of the returns distribution curve, or the higher its kurtosis, the better that market is for momentum strategies.

## Key Points

- Time-Series momentum refers to the positive correlation of a price series' past and future returns.

- Cross-sectional momentum refers to the positive correlation of a price series' past and futuren relative returns, in relation to that of other price series in a portfolio.

- Futures exhibit time series momentum mainly because of the persistence of the sign of roll returns.

- If you are able to find an instrument that cointegrates or correlates with the spot price or return of a commodity, you cna extract the roll return of the commodity future by sorting that instrument during backwardation, or buying that instrument during contango.
...
- Profitable strategies on news sentiment momentum show that the slow diffusion of news is a cause of stock price momentum.

- The contagion of forced asses sales and purchases among mutal funds contributes to stock price momentum.

- Momentum models thrive on "black swan" events and the positive kurtosis of the reutrns distributiond curve.

# Chapter 7 Intraday Momentum Strategies

A long holding periods strateegy present two problems:1.result in lower Sharp Ratios and backtest statistical significance, cause of the infrequent independent trading signals;2.suffer from underperformance in the aftermath of financial crises.

## Opening Gap Strategy

Buy when the instrument gaps up, and short when it gaps down.

**Example 7.1:Opening Gap Strategy for FSTX**

Dow Jones STOXX 50 index futures (FSTX)

The utilities function `smartMovingStd` and `backshift` are available from `epchan.com/book2`.

```{matlab}
entryZscore=0.1;
stdretC2C90d = backshift(1, smartMovingStd(calculateReturns(cl, 1), 90));
longs = op > backshift(1, hi).*(1+extryZsocre*stdretC2C90d);
shorts = op < backshift(1, hi).*(1-extryZsocre*stdretC2C90d);
...
Buy the stock if the return is very postitive and short if the return is very negative, and liquidate the position at the same day's close.

We can use the function `parseEarningsCalendarFromEarningsDotCom` displayed in the box to retrieve one year or so of earnings annoucements data from earnings.com.

The program below carefully selects only earnings announcements occurring after the previous trading day's market close and before today's market open.

**Function for Retrieving Earnings Calendar from earnings.com**

The function takes an input $1 \times N$ array and returns a $1 \times N$ bool array earnann, which tells us whether the corresponding stock has an earnings announcements after the pervious day's 4:00 P.M. ET and before today's 9:30 A.M.ET.

```{matlab}
function [earnann] = parseEarningsCalendarFromEarningsDotCom(prevData, todayData, allsyms)
  ...
  patternPrevDateTime=['<td alig="center"><nobr>', num2str(prevd), '-', num2str(prevmmm), '([ :\dABPMCO]*)</nobr>'];
...
In example 7.2 we calculate the 90-day moving standard deviation of previous-close-to-next day's-open return as the benchmark for deciding whether the announcement is "surprising" enough to generate the post announcement drift.
...
retC20=(op-backshift(1, cl))./backshift(1, cl);
stdC20=smartMovingStd(retC20, lookback);

positions=zeros(size(cl));
longs=retC20 >= 0.5*stdC20 & earnann;
shorts=retC20 <= -0.5*stdC20 & earnann;

positions(lonogs) = 1;
positions(shorts) = -1;

ret=smartsum(positions .* (cl-op)./, 2)/30;
```

With time goes on the duration of momentum is shortened due to increased awareness of the existence of such momentum, so the overnight return of the strategy is negative on average.

### Drift Due to Other Events

In theory, any announcements that a reevaluation of the fair market value of a compay should induce a change in its share price toward a new equilibrium price (Hafez, 2011). Hafez found that the acquiree's stock price falls more than the acquirer's after the initial announcement of the acquisition, which is really weird.

## Leveraged ETF Strategy

A large change in the market index generates momentum in the same direction for bothe leveraged long or short ETFs. If the change is positive, a short ETF would experience a decrease in equity, and its sponsor would need to reduce its short positions. Therefore, it would also need to buy stocks, just as the long ETF would.

## Hight-Frequency Strategies

The basic idea is quite simple: if the bid size is much bigger than the ask size, expect the price to tick up and vice versa.

In markets that fill orders on a pro-rata basis , the simplest way ot benefit from this expectation is just to "join the bid" immediately.为避免损失，we join the bid only when the original bid size is much bigger than the ask size, this is called the ratio trade.

*ticking or quote matching* : place the buy order at the best bid plus one tick, if this is filled, then we place a sell order at the best ask minus one tick and hope that it is filled. If it is not, we can probably still sell it at the original best bid, with the loss of commissions plus one tick.

![FIGURE 7.3 ticking or quote matching](https://cdn.jsdelivr.net/gh/shiwz11/pics_public@master/FMDPics/ATWSR-7-1.png)

High-frequency traders can profit only from slower traders, if only high-frequency traders are left in the market, the net average profit for everyone will be zero.

*Stop hunting* is another favorite high-frequency momentum strategy.Research in the currencies markets indicated that **once support (resistance) levels are breached, prices will go futher down(up) for a while** (Osler, 2000, 2001).This short-term price momentum occures because of the large number of stop orders placed at or near the support and resistance levels.

Empricial research indicates that **order flow** information is a good predictor of price movements. Market makes can distill important fundamental information from order flow information, and set the bid-ask prices accordingly.
...
- "Breakout" momentum strategy involve a price exceeding a trading range.

- The opening gap strategy is a breakout strategy that works for some futures and cueencies.
...
Risk Management is driven by "loss aversion". However, the goal of risk management should be the maximization of long-term equity growth.

The key concept in risk manageent  is the prudent of leverage.

## Optimal Leverage

For those who are answerable to no one but themselves, the sole goal of trading is the maximization of net worth over the long term. They need not to pay attentation to drawdowns and volatilities of returns.

In all cases, we have to make the assumption that the future probability distribution of returns of the market is the same as in the past, which is usually incorrect.Some strategy assume further that the distribution of the return of the strategy itself is the same as in the past. And finally, the most restrictive assumption is that, the probability distribution of the return of the strategy is Gaussian.

No matter how the optimal leverage is determined, the one central theme is that the leverage should be kept constant.However, this sometimes can be quite counterintuitive, for example, if we have a long stock portfolio, and the profit and loss was positive in the last trading period, the constant leverage requirement forces us to buy more stocks for this period.However, if it is negative, it forces us to sell stocks into the loss.

### Kelly Formula

Suppose the distribution of returns is Gaussian, the optimal leverage $f$:
...
$s^2$ : variance of the excess returns.

It can be proved that, if the Gaussian assumption is a good approximation, then the Kelly leverage $f$ will generate the highest compounded growth rate of equity, assuming that all profits are reinvested.

If the overestiamtion of the optimal leverage is high enough, it will ruin us.So, many traders deploy a leverage equal to half of what the Kelly formula recommends: the so-called half-Kelly leverage.In other words, the Kelly Formula better viewed as the upper bound.

Another usage of the Kelly formula: it tell us how to optimally allocate our buying power to different portfolos or strategies.

$$
F= C^{-1}M
$$

$F$ : column vector of optimal leverages;

$C$: covariance matrix of the reuturns of the portfolios;

$M$ : mean excess returns of these portfolios.

If our broker has set a maximum leverage $F_{max}$ that is smaller than the total gross leverage $\sum_{i}^{n} |F_i|$, we usually multiply all $F_i$ by the factor $F_{max}/\sum_{i}^{n} |F_i|$, so than the total gross leverage is equal to $F_{max}$.Another choice is to invest most or all our buying power into the portfolio or strategy with the highest mean excess return.When we have two or more strategies with very differnet independent growth rates, and when we have a maximum leverage constraint that is much lower than the Kelly leverage, it is often optimal to just apply all of our buying power on the strategy that has the highest growth rate.

### Optimization of Expected Growth Rate Using Simulated Returns

We relax the Gaussian assumption, the expected value of the compounded growth rate as a function of the leverage $f$ is (assuming for simplicity that the risk-free rate is zero):

$$
g(f) = <log(1+fR)> \qquad (8.5)
$$

$<\dots>$: average over some random sampling of the unlevered return-per-bar $R(t)$ of the strategy based on some probability distribution of $R$.

Next, we would like to compute Equation 8.5 using a non-Gaussian distribution of R using the Pearson system, which takes as input the mean, standard deviation, skewness, and kurtosis of the empirical distribution of R. Some empirical may have a non-zero higher moments even an infinite higher moments, in this case, we use the Pearson system for our Monte Carlo sampling.

### Optimization of Historical Growth Rate

### Maximum Drawdown

## Constant Proportion Portdolio Insurance

Constant proportion portfolio insurance (CPPI): one method to maximize compounded growth while limiting the maximum drawdown.

It is quite simple, suppose the optimal Kelly leverage of our strategy is determined to be $f$, and the maximum drawdown is $-D$. We simply set aside D of our initial total account equity for trading, and apply a leverage of $f$ to this subaccount, the other $1-D$ of the account will be sitting in cash. If our total account reach a higher water mark we reset our subaccount so that it is again D of the total equity. And of course, even we loss all the money in the subaccount the maximum drawdown wouldn't be higher than $-D$.

**Computing Growth Rate Using CPPI**:

```{matlab}
# assume the return series is ret_sim
# the optimal leverage is optimalF
# maximum drawdown allowed is -D=-0.5
g_cppi = 0;
drawdown = 0;
D = 0.5;
...
Stop loss can only prevent the unrealized $P\&L$ from exceeding our self-imposed limit if the market is always open whenever we are holding a position.


## Risk Indicators

Leading risk indicators let us predict whether the next period will be risky for our investment.

For example, the TED spread, which is the difference between the three-month London Interbank Offered Rate (LIBOR) and the three-month T-bill interest rate, and it measures the risk of bank defaults.
...
Instad of blindly adding more rules, more indicators to the model and hoping that they miraculously improve the model performance, we **should look for a fundamaental reason and then quantitatively test whether this fundamental reason is valid.**

Beware of the regime shift, the fundamental change in the market structure due to government regulatory or macroeconomic changes.
